...
1. Read the PRD at the exact path provided in the prompt preamble (do not assume `prd.json` is in the current repo)
2. Read the progress log at the exact path provided in the prompt preamble (check Codebase Patterns section first)
...
9. Update the PRD file path from the prompt preamble to set `passes: true` for the completed story
10. Append your progress to the progress file path from the prompt preamble
